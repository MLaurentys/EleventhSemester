...
import { SwFooter } from './sw-footer.js';
import { SwCard } from './sw-card.js';
...
const showCharacter = (swcObj) => {
  ...
  swCard.dataset.masters = JSON.stringify(swcObj.masters) ?? '[]';
...
const jsonLoaded = (json) => {
  ...
  const optionHTML = Object.keys(swcJSON)
    .map(
      (swc) =>
        `<option value="${swcJSON[swc].id}">${swcJSON[swc].name}</option>`
    )
    .join('');
